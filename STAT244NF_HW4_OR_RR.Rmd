...
6. This homework assignment is **due on Friday, October 1, 2021 and should be submitted to Gradescope**.
...
(a) Fit a Poisson regression model with winter festival as an explanatory variable to these data. Remember to use the `glm` function and to specify `family` argument in the `glm` function as `poisson`. Assign the model fit to `outbreak_pois` and print the summary of the model fit.

```{r}

```

(b) What is the estimate effect of winter festival? Exponetiate this estimate. How does this compare to the quantities you calculated in Question ID 3? Interpret this value in context.
...
(a) Fit a logistic regression model with winter festival as an explanatory variable to these data. Remember to use the `glm` function and to specify `family` argument in the `glm` function as `binomial`. Assign the model fit to `outbreak_binom` and print the summary of the model fit.

```{r}

```

(b) What is the estimate of the effect of winter festival? Exponentiate this estimate. 
...
#### Question NSF 6. `rbinom(n = N_d, size = 1, prob = N_fest/N_d)` and `rbinom(size = 1, n = N_d, prob = N_fest/N_d)` do not give different outputs. Why?